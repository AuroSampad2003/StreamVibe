...
function NewReleases() {
  ...
  return (
    <div className="text-white px-20 xl:px-10 md:px-6 sm-max:px-0.5 mt-16 mb-16">
      <h2 className="font-bold text-2xl sm:text-xl mb-6">New Releases</h2>
      <Carousel responsive={responsive} className="py-3">
        {
          data.map((item, index) => {
            return (
              <Link to={`/${item.id}`} key={index}  >
                <div className='border border-black5  bg-black3 rounded-2xl mx-2 p-4'>
                  ...
                  <h1 className='text-center text-sm sm:text-base text-gray1 bg-black2 py-1 w-fit mx-auto px-2 mt-1 border border-black5 rounded-3xl'>Realeased at {item.release_date}</h1>
                </div>
              </Link>
            )
          })
