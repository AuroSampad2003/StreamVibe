      ...
      try {
        ...
        const detailedData = await Promise.all(
          json.results.map(async (tv) => {
            ...
            return {
              ...tv,
              // runtime: details.number_of_episodes && details.episode_run_time?.length > 0
              //   ? details.number_of_episodes * details.episode_run_time[0]
              //   : null,
  ...
  return (
    <div className="text-white px-20 xl:px-10 md:px-6 sm-max:px-0.5 mt-16 mb-16">
      <div className="flex justify-between items-center mb-6">
        <h2 className="font-bold text-2xl lg:text-2xl sm:text-xl">Trending Shows Now</h2>
      ...
      >
        {data.map((item, index) => (
          <div
            key={index}
            onClick={() => navigate(`/tv/${item.id}`)} // Use navigate here
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-3 gap-1 sm:gap-4">
              {/* Runtime */}
              ...
              <div className="flex items-center gap-1 px-2 py-[2px] rounded-full bg-[#141414] border border-[#262626] text-[#999999] text-xs sm:text-sm w-fit">
                ...
                {item.number_of_seasons > 1
                  ? `${item.number_of_seasons} Sns`
                  : `${item.number_of_seasons} Sn`}
              </div>
            </div>
