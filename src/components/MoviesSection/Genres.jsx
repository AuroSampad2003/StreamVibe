...
function Genres({ categoryType = "movie" }) {
  ...
  const filteredGenres = genresList.filter((genre) => {
    return categoryType === "movie"
      ? ![10770, 37].includes(genre.id)
      : ![99, 10402, 9648].includes(genre.id);
  });
    ...
    <div className="text-white px-20 xl:px-10 sm-max:px-0.5 mt-8 mb-16">
      ...
      <Carousel
        ...
        beforeChange={(nextSlide) =>
          setCurrentIndex(nextSlide % Math.max(filteredGenres.length, 1))
        }
      >
        {filteredGenres.length > 0 ? (
          filteredGenres.map((genre, index) => (
            ...
            >
              <div className="grid grid-cols-2 gap-2">
                {genresDetails.filter((item) => item.genre_ids.includes(genre.id)).slice(0, 4)
                  .length > 0 ? (
                  genresDetails
